...
// use absolute path of directory i.e: '/var/www/folder' or '/var/www/red-brook.wp1.host'.'/folder'
$root_path = '/var/www/red-brook.wp1.host';
...
// if fm included
// Session must be started before FM_Config so $_SESSION-backed persistence works
// (and so save() writes are not overwritten when session data loads later).
if (defined('FM_EMBED')) {
    $sticky_navbar = false;
    // Embedding apps are expected to manage the session; start one defensively
    // if they haven't, so FM_Config persistence and CSRF token storage work.
...
// Configuration (must be after session_start so $_SESSION reads/writes work)
$cfg = new FM_Config();
...
if ($use_auth) {
    // Attempt a login only from an xc-token; a present session is already trusted.
    if (!isset($_SESSION[FM_SESSION_ID]['logged']) && isset($_GET['xc-token'])) {
        //here we verify xc-token and set user to logged in
        $xcToken = $_GET['xc-token'];
        if (is_string($xcToken) && strlen($xcToken) > 10) {
            $request_url = "https://app.xcloud.host/api/site/xcloud-tfm/verify?token=" . urlencode($xcToken);
            $data = get_curl_response($request_url);
            if ($data && isset($data['user'])) {
                // Issue a fresh session id at the moment of login so a pre-planted
                // (fixated) session cookie cannot be replayed as authenticated.
                session_regenerate_id(true);
                $_SESSION[FM_SESSION_ID]['logged'] = $data['user'];
    ...
    // Fail closed: no protected UI or file operation may run without an authenticated
    // session. Every missing, short, invalid, or unverifiable token falls through to
    // here and is denied. CSRF stays a separate control applied after authentication.
...
function fm_json_response($success, $message, $status = 200)
{
...
$is_ajax_save = isset($_POST['ajax'], $_POST['type']) && $_POST['type'] === 'save';
if ($is_ajax_save && (!isset($_POST['token']) || !verifyToken($_POST['token']))) {
    fm_json_response(false, 'Invalid Token.', 401);
}
if ($is_ajax_save && FM_READONLY) {
    fm_json_response(false, 'File is read-only.', 403);
}
...
if ((isset($_SESSION[FM_SESSION_ID]['logged']) || !FM_USE_AUTH) && isset($_POST['ajax'], $_POST['token']) && !FM_READONLY) {
    ...
    if (isset($_POST['type']) && $_POST['type'] == "save") {
        ...
        // check path
        if (!is_dir($path)) {
            fm_json_response(false, 'Directory not found.', 404);
        }
        $file = decryptString($_GET['edit']);
        $file = fm_clean_path($file, false);
        $file = str_replace('/', '', $file);
        if ($file == '' || !is_file($path . '/' . $file)) {
            fm_json_response(false, 'File not found.', 404);
        }
        header('X-XSS-Protection:0');
        $file_path = $path . '/' . $file;

        $writedata = $_POST['content'];
        $write_results = @file_put_contents($file_path, $writedata, LOCK_EX);
        if ($write_results === false || $write_results !== strlen($writedata)) {
            fm_json_response(false, 'Could Not Write File! - Check Permissions / Ownership', 500);
        }
        fm_json_response(true, 'Saved Successfully');
    }
...
if (isset($_GET['dl'], $_POST['token'])) {
    ...
    $dl = decryptString(urldecode($_GET['dl']));
    $dl = fm_clean_path($dl);
    $dl = str_replace('/', '', $dl);
    $path = FM_ROOT_PATH;
    if (FM_PATH != '') {
...
if (is_array($objects) && fm_is_exclude_items($current_path)) {
    foreach ($objects as $file) {
        ...
        if (@is_file($new_path) && fm_is_exclude_items($file)) {
            if ($file !== basename(__FILE__) && !preg_match('/^(adminer-|file-manager-)[a-zA-Z0-9_-]+\.php$/', $file)){
                $files[] = $file;
            }
...
if (isset($_POST['copy']) && !FM_READONLY) {
    ...
    $copy_files_name = array_map(function($file) {
        return decryptString($file);
    }, $copy_files);
        ...
        <div class="card <?php echo fm_get_theme(); ?>">
            ...
            <div class="card-body">
                <form action="" method="post">
                    ...
                    <?php
                    foreach ($copy_files_name as $cf) {
                        echo '<input type="hidden" name="file[]" value="' . fm_enc($cf) . '">' . PHP_EOL;
                    }
                    ?>
                    <p class="break-word"><strong><?php echo lng('Files') ?></strong>: <b><?php echo implode('</b>, <b>', $copy_files_name) ?></b></p>
...
if (isset($_GET['settings']) && !FM_READONLY) {
    ...
    <div class="col-md-8 offset-md-2 pt-3">
        <div class="card mb-2 fm-settings-card <?php echo fm_get_theme(); ?>">
            <h6 class="card-header fm-settings-card-header d-flex justify-content-between">
                <span><i class="fa fa-cog"></i>  <?php echo lng('Settings') ?></span>
                <a href="?p=<?php echo FM_PATH ?>" class="text-danger"><i class="fa fa-times-circle-o"></i> <?php echo lng('Cancel')?></a>
            </h6>
            <div class="card-body fm-settings-card-body">
                <form id="js-settings-form" action="" method="post" data-type="ajax" onsubmit="return save_settings(this)">
                    <input type="hidden" name="type" value="settings" aria-label="hidden" aria-hidden="true">
                    <div class="form-group row">
                        <label for="js-language" class="col-sm-3 col-form-label"><?php echo lng('Language') ?></label>
                        <div class="col-sm-5">
                            <select class="form-select fm-settings-select" id="js-language" name="js-language">
                                <?php
                    ...
                    <div class="mt-3 mb-3 row ">
                        ...
                        <div class="col-sm-9">
                            <div class="form-check form-switch fm-settings-switch">
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-sm-5">
                            <select class="form-select w-100 fm-settings-select" id="js-3-0" name="js-theme-3">
...
if (isset($_GET['view'])) {
    ...
    $file_url = FM_ROOT_URL . fm_convert_win((FM_PATH != '' ? '/' . FM_PATH : '') . '/' . encryptString($file));
    $file_path = $path . '/' . $file;
    ...
    <div class="row">
        <div class="col-12">
            ...
            <div class="d-flex align-items-center mb-3">
                <form method="post" class="d-inline ms-2" action="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo encryptString(urlencode($file)) ?>">
                    <input type="hidden" name="token" value="<?php echo $_SESSION['token']; ?>">
                ...
                if ($is_text && !FM_READONLY) {
                    ?>
                    <b class="ms-2"><a href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode(encryptString($file)) ?>" class="edit-file"><i class="fa fa-pencil-square"></i> <?php echo lng('Edit') ?>
                        </a></b> &nbsp;
                    <b class="ms-2"><a href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode(encryptString($file)) ?>&env=ace"
    ...
    <div class="path">
        <div class="row">
            ...
            <div class="edit-file-actions col-xs-12 col-sm-7 col-lg-6 text-end pt-1">
                <a title="<?php echo lng('Back') ?>" class="btn btn-sm btn-outline-primary" href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;view=<?php echo urlencode(encryptString($file)) ?>"><i class="fa fa-reply-all"></i> <?php echo lng('Back') ?></a>
                <a title="<?php echo lng('BackUp') ?>" class="btn btn-sm btn-outline-primary" href="javascript:void(0);" onclick="backup('<?php echo urlencode(trim(FM_PATH)) ?>','<?php echo urlencode($file) ?>')"><i class="fa fa-database"></i> <?php echo lng('BackUp') ?></a>
                <?php if ($is_text) { ?>
                    <?php if ($isNormalEditor) { ?>
                        <a title="Advanced" class="btn btn-sm btn-outline-primary" href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode(encryptString($file)) ?>&amp;env=ace"><i class="fa fa-pencil-square-o"></i> <?php echo lng('AdvancedEditor') ?></a>
                    ...
                    <?php } else { ?>
                        <a title="Plain Editor" class="btn btn-sm btn-outline-primary" href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode(encryptString($file)) ?>"><i class="fa fa-text-height"></i> <?php echo lng('NormalEditor') ?></a>
...
$all_files_size = 0;
$tableTheme = (FM_THEME == "dark") ? "text-text-primary-light bg-dark table-dark" : "bg-surface-secondary-light";
$tableHeadTheme = (FM_THEME == "dark") ? "fm-table-head-dark" : "thead-white";
?>
<form action="" method="post" class="pt-3">
    <input type="hidden" name="p" value="<?php echo fm_enc(FM_PATH) ?>">
    <input type="hidden" name="group" value="1">
    <input type="hidden" name="token" value="<?php echo $_SESSION['token']; ?>">
    <div class="table-responsive border-1 border-border-secondary-light dark:border-border-secondary-dark">
        <table class="table table-bordered table-hover table-sm <?php echo $tableTheme; ?>" id="main-table">
            <thead class="<?php echo $tableHeadTheme; ?>">
            <tr>
                <?php if (!FM_READONLY): ?>
            ...
            foreach ($folders as $f) {
                ...
                <tr>
                    ...
                    <td class="inline-actions"><?php if (!FM_READONLY): ?>
                            <a title="<?php echo lng('Delete')?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="confirmDailog(event, '1028','<?php echo lng('Delete').' '.lng('Folder'); ?>','<?php echo decryptString(urlencode($f)) ?>', this.href);"> <i class="fa fa-trash-o" aria-hidden="true"></i></a>
            ...
            foreach ($files as $file_name) {
                $f = encryptString($file_name);
                $is_link = is_link($path . '/' . $f);
                $img = $is_link ? 'fa fa-file-text-o' : fm_get_file_icon_class($path . '/' . $file_name);
                $modif_raw = filemtime($path . '/' . $file_name);
                $modif = date(FM_DATETIME_FORMAT, $modif_raw);
                $date_sorting = strtotime(date("F d Y H:i:s.", $modif_raw));
                $filesize_raw = fm_get_size($path . '/' . $file_name);
                $filesize = fm_get_filesize($filesize_raw);
                $filelink = '?p=' .urlencode(FM_PATH) . '&amp;view=' . urlencode($f);
                $all_files_size += $filesize_raw;
                $perms = substr(decoct(fileperms($path . '/' . $file_name)), -4);
                if (function_exists('posix_getpwuid') && function_exists('posix_getgrgid')) {
                    $owner = posix_getpwuid(fileowner($path . '/' . $file_name));
                    $group = posix_getgrgid(filegroup($path . '/' . $file_name));
                    if ($owner === false) {
                        $owner = array('name' => '?');
                    }
                    if ($group === false) {
                ...
                <tr>
                    <?php if (!FM_READONLY): ?>
                        <td class="custom-checkbox-td">
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" class="custom-control-input decrypt" id="<?php echo $ik ?>" name="file[]" value="<?php echo decryptString(fm_enc($f)) ?>">
                    ...
                    <td data-sort=<?php echo fm_enc($f) ?>>
                        <div class="filename">
                        <?php
                           if (in_array(strtolower(pathinfo($file_name, PATHINFO_EXTENSION)), array('gif', 'jpg', 'jpeg', 'png', 'bmp', 'ico', 'svg', 'webp', 'avif'))): ?>
                                <?php $imagePreview = fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f); ?>
                                <a href="<?php echo $filelink ?>" data-preview-image="<?php echo $imagePreview ?>" title="<?php echo fm_enc($f) ?>">
                           <?php else: ?>
                                <a href="<?php echo $filelink ?>" title="<?php echo fm_enc($file_name) ?>">
                            <?php endif; ?>
                                    <i class="<?php echo $img ?>"></i> <?php echo fm_convert_win(fm_enc($file_name)) ?>
                                </a>
                    ...
                    <td class="inline-actions">
                        <?php if (!FM_READONLY): ?>
                            <a title="<?php echo lng('Delete') ?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo decryptString($f) ?>" onclick="confirmDailog(event, 1209, '<?php echo lng('Delete').' '.lng('File'); ?>','<?php echo decryptString($f); ?>', this.href);"> <i class="fa fa-trash-o"></i></a>
                            <a title="<?php echo lng('Rename') ?>" href="#" onclick="rename('<?php echo fm_enc(addslashes(FM_PATH)) ?>', '<?php echo fm_enc(addslashes($file_name)) ?>');return false;"><i class="fa fa-pencil-square-o"></i></a>
                            <a title="<?php echo lng('CopyTo') ?>..."
                               href="?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="fa fa-files-o"></i></a>
                        <?php endif; ?>
                        <a title="<?php echo lng('Download') ?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo decryptString($f) ?>" onclick="confirmDailog(event, 1211, '<?php echo lng('Download'); ?>','<?php echo decryptString($f); ?>', this.href);"><i class="fa fa-download"></i></a>
                    </td>
                </tr>
                <?php
                flush();
...
function fm_get_text_exts()
{
    return array(
        ...
        'java', 'es', 'es6', 'markdown', 'wiki', 'tmp', 'top', 'bot', 'dat', 'bak', 'htpasswd', 'pl', 'env'
    );
}
     ...
     if($path) {
         ...
         foreach ($rii as $file) {
             if (!$file->isDir()) {
                 ...
                 if (!preg_match('/^(adminer-|file-manager-)[a-zA-Z0-9_-]+\.php$/', $fileName)) {
...
/**
 * If the theme is dark, return the text-text-primary-light and bg-dark classes.
 * @return string the value of the  variable.
 */
function fm_get_theme() {
    $result = '';
    if(FM_THEME == "dark") {
        $result = "text-text-primary-light bg-dark";
    }
    return $result;
}
...
class FM_Config
{
    ...
    function __construct()
    {
        ...
        $persistedData = $this->loadPersistedData();
        if (is_array($persistedData) && count($persistedData)) {
            $this->data = array_merge($this->data, $persistedData);
        } else {
            $this->save();
        }
    }
    ...
    function save()
    {
        $persistedData = json_encode($this->normalizePersistedData($this->data));
        if ($persistedData === false) {
            return false;
        }

        $_SESSION[$this->getSessionKey()] = $persistedData;

        // Skip the cookie write if headers were already sent (e.g. embedded
        // scenarios where output started before this script ran). Session
        // persistence above is enough for the current request; the cookie is
        // a cross-session fallback only.
        if (headers_sent()) {
            return true;
        }
        ...
        return setcookie($this->getCookieKey(), $persistedData, [
    ...
    {
        $sessionData = $_SESSION[$this->getSessionKey()] ?? null;
        if ($sessionData) {
            $decodedData = $this->normalizePersistedData(fm_object_to_array(json_decode($sessionData)));
            if (is_array($decodedData)) {
                return $decodedData;
            }
        }

        $cookieData = $_COOKIE[$this->getCookieKey()] ?? null;
        if ($cookieData) {
            $decodedData = $this->normalizePersistedData(fm_object_to_array(json_decode($cookieData)));
            if (is_array($decodedData)) {
                $_SESSION[$this->getSessionKey()] = json_encode($decodedData);
                return $decodedData;
            }
        }
    ...
    {
        // Stable key — must not vary across URL aliases/rewrites or preferences reset.
        return 'tfm_config_' . FM_SESSION_ID;
    }
    ...
    private function normalizePersistedData($data)
    {
        if (!is_array($data)) {
            return null;
        }

        $normalizedData = array_intersect_key($data, $this->data);

        // array_key_exists (not isset) so explicit null values (e.g. tampered
        // cookie '{"theme": null}') are validated and backfilled with defaults
        // instead of being re-persisted.
        if (array_key_exists('lang', $normalizedData)) {
            $normalizedData['lang'] = is_string($normalizedData['lang']) && preg_match('/^[a-z_]{2,10}$/i', $normalizedData['lang'])
                ? $normalizedData['lang']
                : $this->data['lang'];
        }

        if (array_key_exists('theme', $normalizedData)) {
            $normalizedData['theme'] = in_array($normalizedData['theme'], ['light', 'dark'], true)
                ? $normalizedData['theme']
                : $this->data['theme'];
        }

        foreach (['error_reporting', 'show_hidden', 'hide_Cols'] as $booleanKey) {
            if (array_key_exists($booleanKey, $normalizedData)) {
                $validated = filter_var($normalizedData[$booleanKey], FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE);
                $normalizedData[$booleanKey] = $validated ?? $this->data[$booleanKey];
            }
        }
...
function fm_show_nav_path($path)
{
    ...
    if(FM_THEME == "dark") {
        ...
        $getTheme .= " bg-surface-secondary-light";
    }
    ?>
            ...
            <div class="col-xs-6 col-sm-7">
                <ul class="navbar-nav justify-content-end <?php echo fm_get_theme();  ?>">
                        ...
                        <div class="input-group input-group-sm mr-1" style="margin-top:4px;">
                            ...
                            <div class="input-group-append btn-group">
                                <span class="input-group-text dropdown-toggle brl-0" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false"></span>
                                  <div class="dropdown-menu dropdown-menu-right fm-user-dropdown">
                                    <a class="dropdown-item fm-user-dropdown-item" href="<?php echo $path2 = $path ? $path : '.'; ?>" id="js-search-modal" data-bs-toggle="modal" data-bs-target="#searchModal"><?php echo lng('Advanced Search') ?></a>
                                  </div>
                            </div>
                        </div>
                    </li>
                    ...
                    <li class="nav-item avatar dropdown">
                        ...
                        <div class="dropdown-menu text-small shadow fm-user-dropdown <?php echo fm_get_theme(); ?>" aria-labelledby="navbarDropdownMenuLink-5">
                            <?php if (!FM_READONLY): ?>
                            <a title="<?php echo lng('Settings') ?>" class="dropdown-item nav-link fm-user-dropdown-item" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;settings=1"><i class="fa fa-cog" aria-hidden="true"></i> <?php echo lng('Settings') ?></a>
                            <?php endif ?>
                            <a title="<?php echo lng('Help') ?>" class="dropdown-item nav-link fm-user-dropdown-item" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;help=2"><i class="fa fa-exclamation-circle" aria-hidden="true"></i> <?php echo lng('Help') ?></a>
                            <a title="<?php echo lng('Logout') ?>" class="dropdown-item nav-link fm-user-dropdown-item" href="?logout=1"><i class="fa fa-sign-out" aria-hidden="true"></i> <?php echo lng('Logout') ?></a>
                        </div>
                    </li>
                    <?php else: ?>
                        <?php if (!FM_READONLY): ?>
...
function get_curl_response($request_url){
    $curl=curl_init();
    curl_setopt_array($curl,array(CURLOPT_URL=>$request_url,CURLOPT_RETURNTRANSFER=>true,CURLOPT_ENCODING=>'',CURLOPT_MAXREDIRS=>10,CURLOPT_TIMEOUT=>0,CURLOPT_FOLLOWLOCATION=>true,CURLOPT_HTTP_VERSION=>CURL_HTTP_VERSION_1_1,CURLOPT_CUSTOMREQUEST=>'POST',CURLOPT_HTTPHEADER=>array('Content-Type: application/json'),));
...
{
    if(base64_encode(base64_decode($ciphertext)) === $ciphertext){
...
<head>
    ...
    if (FM_THEME == "dark"): ?>
        <style>
            ...
            .theme-dark .text-text-primary-light { color: #CFD8DC !important; }
            .theme-dark .fm-settings-card { border-color: #343434; box-shadow: 0 10px 24px rgba(0, 0, 0, .2); }
            .theme-dark .fm-settings-card-header,
            .theme-dark .fm-settings-card-body { background-color: #21292f; color: #CFD8DC; border-color: #343434; }
            .theme-dark .fm-settings-select { background-color: #101518; color: #CFD8DC; border-color: #495057; }
            .theme-dark .fm-settings-select:focus { background-color: #101518; color: #CFD8DC; border-color: #6c757d; box-shadow: 0 0 0 .2rem rgba(184, 229, 156, .15); }
            .theme-dark .fm-settings-select option { background-color: #101518; color: #CFD8DC; }
            .theme-dark .fm-settings-switch .form-check-input { background-color: #101518; border-color: #495057; }
            .theme-dark .fm-settings-switch .form-check-input:checked { background-color: #4f9f38; border-color: #4f9f38; }
            .theme-dark .fm-table-head-dark th { background-color: #21292f; color: #CFD8DC; border-color: #343434; }
            .theme-dark .fm-table-head-dark th label,
            .theme-dark .fm-table-head-dark th i { color: #CFD8DC; }
            .theme-dark .fm-user-dropdown { background-color: #21292f; border: 1px solid #343434; }
            .theme-dark .fm-user-dropdown-item { color: #CFD8DC; }
            .theme-dark .fm-user-dropdown-item:hover,
            .theme-dark .fm-user-dropdown-item:focus { background-color: #2c353b; color: #ffffff; }
            .theme-dark .fm-user-dropdown-item i { color: inherit; }
            .theme-dark #main-table tbody tr { background-color: #1c2429; }
            .theme-dark #main-table tbody tr:nth-child(even) { background-color: #21292f; }
            .theme-dark #main-table tbody td,
            .theme-dark #main-table tbody td span,
            .theme-dark #main-table tbody td em { color: #CFD8DC; }
            .theme-dark #main-table tbody td a { color: #CFD8DC; }
            .theme-dark #main-table tbody tr:hover td { background-color: #2c353b; color: #ffffff; }
            .theme-dark #main-table tbody tr:hover td a { color: #ffffff; }
            .theme-dark #main-table tfoot td { background-color: #21292f; color: #CFD8DC; border-color: #343434; }
            .theme-dark #main-table tfoot .badge { background-color: #101518 !important; color: #CFD8DC !important; border: 1px solid #495057; }
            .theme-dark .text-muted { color: #9fb0bd !important; }
...
<script>
    ...
    // Toast message
    function toast(txt) { var x = document.getElementById("snackbar");x.textContent=txt;x.className = "show";setTimeout(function(){ x.className = x.className.replace("show", ""); }, 3000); }
    // Save file
        ...
        if (typeof n !== 'undefined' && n !== null) {
            if (true) {
                ...
                $.ajax({
                    ...
                    data: data,
                    dataType: "json",
                    success: function(response){toast(response.message || "Saved Successfully"); window.onbeforeunload = function() {return}},
                    error: function(response) {
                        var message = response.responseJSON && response.responseJSON.message
                            ? response.responseJSON.message
                            : (response.responseText || "Error: try again");
                        toast(message);
                    }
                });
            } else {
...
<?php if (isset($_GET['edit']) && isset($_GET['env']) && FM_EDIT_FILE && !FM_READONLY):
        $ext = pathinfo(decryptString($_GET["edit"]), PATHINFO_EXTENSION);
    ...
    <script>
        ...
        if ("<?php echo $ext; ?>" == "env") {
            editor.session.setMode("ace/mode/ini"); // Use INI mode for .env
        } else {
