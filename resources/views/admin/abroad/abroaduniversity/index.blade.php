...
@section('content')
    ...
    <div class="card">
        <div class="card-header justify-content-between align-items-center">
            <h5 class="mb-0">Abroad University ({{ $university->total() }})</h5>
            <small class="text-muted float-end" style="margin-left: 10px;">
                <a href="{{ route('abroaduniversity.create', $abroad_id) }}" class="btn btn-primary"><i class="fa-solid fa-plus"></i>
                    Create</a>
            </small>
            <small class="text-muted float-end  " >
                <a href="{{ route('abroad.index') }}" class="btn btn-primary"><i class="fa-solid fa-backward"></i>
                    Back</a>
            </small>
            ...
            @if (!$university->isEmpty())
                <table class="table">
                    ...
                    <tbody class="table-border-bottom-0">
                        @foreach ($university as $key => $blog)
                            <tr>
                                <td><strong>{{ $key + $university->firstItem() }}</strong></td>
                                <td class="">
                                ...
                                <td>
                                    ...
                                    <a href="{{ route('abroaduniversity.edit', [$abroad_id,$blog->id]) }}"
                                        style="float: left;margin-right: 5px;" class="btn btn-sm btn-primary"><i
                                            class="fa-solid fa-pen-to-square"></i> Edit</a>


                                    <form class="delete-form" action="{{ route('abroaduniversity.destroy',[$abroad_id, $blog->id]) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
