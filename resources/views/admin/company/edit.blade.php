...
@section('title', 'Edit ' . $company->name . ' - BG Group')
        ...
        <div class="card container-fluid mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <small class="text-muted float-end">
                    <a class="btn btn-primary" href="{{ route('company.index') }}"><i class="fa-solid fa-arrow-left"></i>
                        Back</a>
                </small>
            </div>
            <div class="card-body p-0">
                <form class="row" method="POST" action="{{ route('company.update', $company->id) }}"
                        ...
                        <div class="card card-body main-description shadow br-8 p-4">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group mb-3">
                                        ...
                                        <input class="form-control @error('slug') is-invalid @enderror" id=""
                                            type="text" name="slug" value="{{ old('slug', $company->slug) }}"
                                            placeholder="">
                                        @error('slug')
                                            <div class="invalid-feedback" style="display: block;">
                                                {{ $message }}
                                            ...
                                            @if ($category->isNotEmpty())
                                                @foreach ($category as $key => $value)
                                                    <option class="p-3" value="{{ $value->id }}"
                                                        {{ $value->id == $company->category ? 'selected' : '' }}>
                                                        {{ $value->name }}
                                                    </option>
                                                @endforeach
                                            @endif
                                ...
                                <div class="col-md-4">
                                    <div class="form-group mb-3">
                                        ...
                                        <input class="form-control br-8 @error('date') is-invalid @enderror" type="text"
                                            name="date" value="{{ old('date', $company->date) }}"
                            ...
                            <div class="form-group mb-3">
                                ...
                                <textarea class="form-control ckeditor br-8 @error('description') is-invalid @enderror" id="description"
                                    name="description" rows="10" placeholder="Enter Description">{{ old('description', $company->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback" style="display: block;">
                                        {{ $message }}
                                    </div>
                    ...
                    <div class="col-md-3">
                        <div class="card-body card shadow br-8">
                            <div class="form-group mb-3 d-flex align-items-center">
                                ...
                                <select class="form-select ms-5" id="status" name="status">
                                    <option class="p-3" value="0" @if ($company->status == 0) selected @endif>
                                        Draft</option>
                                    <option class="p-3"@if ($company->status == 1) selected @endif value="1">
                                        Publish</option>
                                </select>
                            </div>
                            <hr class="shadow-sm">
                            ...
                            <div class="form-group mb-3 mt-2">
                                ...
                                @if ($company->image)
                                    <img class="mt-2 old-image"
                                        src="{{ asset('admin/images/company/' . $company->image) }}" width="100">
                                @endif
                                @error('image')
                                    <div class="invalid-feedback" style="display: block;">
                                        {{ $message }}
                        ...
                        <div class="form-group mb-3 d-flex align-items-center">
                            ...
                            <input class="form-control ms-5 @error('order') is-invalid @enderror" type="number"
                                name="order" value="{{ old('order', $company->order) }}" placeholder="Enter Order">
                            @error('order')
                                <div class="invalid-feedback" style="display: block;">
                                    {{ $message }}
                                </div>
                        ...
                        <div class="card-footers d-flex justify-content-between">
                            {{-- <a class="btn btn-sm btn-success" href="{{ route('projectsingle', $company->slug) }}"
